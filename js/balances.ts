...
function computeBalances(expenses: Expense[], currency?: string): Record<string, Money> {
  ...
  function bump(id: string, delta: Money) {
...
function simplifyDebts(balances: Record<string, Money>): { from: string; to: string; amount: Money }[] {
  ...
  const transfers: { from: string; to: string; amount: Money }[] = [];
  let ci = 0;
  let di = 0;
...
interface SettlementSummary {
  ...
  transfers: { from: string; to: string; amount: Money }[];
...
function debtMatrix(expenses: Expense[], currency?: string): Record<string, Record<string, Money>> {
  ...
  function owe(debtor: string, creditor: string, amount: Money) {
...
function totalTransferred(transfers: { from: string; to: string; amount: Money }[]): Money {
...
function validateSettlement(balances: Record<string, Money>, transfers: { from: string; to: string; amount: Money }[]): SettlementValidation {
