...
function saveGroup(group: Group): boolean {
  try {
...
function loadGroup(): Group | null {
  try {
    ...
    const expenses =
      (parsed.expenses as any[] || []).map((e: any) =>
        Object.assign({}, e, {
...
async function exportGroupToJSON(group: Group): Promise<string> {
  const backup = await import("./backup.js");
  return JSON.stringify(backup.exportGroup(group), null, 2);
}

async function importGroupFromJSON(text: string): Promise<Group> {
  const backup = await import("./backup.js");
