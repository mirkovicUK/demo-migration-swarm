...
export function validateExpense(input: unknown): ValidationProblem[] {
  ...
  if (!Array.isArray(inp.participants) || inp.participants.length === 0) {
    ...
    if (inp.paidBy && inp.participants.indexOf(inp.paidBy) === -1) {
...
export function validateSplit(
  ...
  const sp = split as any;
  if (!isSplitKind(sp.kind)) {
  ...
  if (!sp.values || typeof sp.values !== "object") {
    problems.push(
      problem("NO_SPLIT_VALUES", "split", sp.kind + " split needs values")
    );
    return problems;
  }
  ...
  if (sp.kind === "percentage") {
    const totalPct = sumValues(sp.values);
  ...
  if (sp.kind === "exact" && isMoney(amount)) {
    const total = sumExactValues(sp.values, (amount as Money).currency);
