import { Money, zero, add, isPositive, isZero, equals, compare, DEFAULT_CURRENCY, isMoney } from "./money.js";
...
export interface ExpenseInput {
  ...
  paidBy: string;
  participants: string[];
  split: { kind: string; values?: Record<string, unknown> };
...
export function validateExpense(input: unknown): ValidationProblem[] {
  ...
  const typedInput = input as ExpenseInput;
  ...
  if (!Array.isArray(typedInput.participants) || typedInput.participants.length === 0) {
    ...
    if (typedInput.paidBy && typedInput.participants.indexOf(typedInput.paidBy) === -1) {
...
export function validateSplit(split: unknown, participants: string[] | undefined, amount: unknown): ValidationProblem[] {
  ...
  const typedSplit = split as { kind: string; values?: Record<string, unknown> };
  ...
  if (!typedSplit.values || typeof typedSplit.values !== "object") {
    problems.push(problem("NO_SPLIT_VALUES", "split", typedSplit.kind + " split needs values"));
    return problems;
  }
...
function normalizeSplit(split: { kind: string; values?: Record<string, unknown> }, participants: string[]): { kind: string; values?: Record<string, unknown> } {
  if (split.kind === "equal") {
    return { kind: "equal" };
  }
