...
export function validateExpense(input: unknown): ValidationProblem[] {
  ...
  if (!Array.isArray(typedInput.participants) || typedInput.participants.length === 0) {
    ...
    if (typedInput.paidBy && !typedInput.participants.includes(typedInput.paidBy)) {
...
export function validateSplit(split: unknown, participants: string[] | undefined, amount: unknown): ValidationProblem[] {
  ...
  if (!splitObj.values || typeof splitObj.values !== "object") {
    problems.push(problem("NO_SPLIT_VALUES", "split", `${splitObj.kind} split needs values`));
    return problems;
  }
  ...
  if (splitObj.kind === "exact" && isMoney(amount)) {
    const total = sumExactValues(splitObj.values, amount.currency);
