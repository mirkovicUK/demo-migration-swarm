...
export function isSplitKind(value: unknown): value is SplitKind {
  return typeof value === 'string' && SPLIT_KINDS.indexOf(value) !== -1;
}
...
export function validateExpense(input: unknown): ValidationProblem[] {
  ...
  if (!Array.isArray(inp.participants) || inp.participants.length === 0) {
    ...
    if (hasDuplicates(inp.participants)) {
      problems.push(
  ...
  const participants = Array.isArray(inp.participants) ? inp.participants as string[] : undefined;
  const splitProblems = validateSplit(inp.split, participants, inp.amount);
...
export function validateSplit(split: unknown, participants: string[] | undefined, amount: unknown): ValidationProblem[] {
  ...
  const kind = s.kind as SplitKind;
  if (kind === "equal") {
    return problems; // no per-participant values needed
  }
  if (!s.values || typeof s.values !== "object") {
    problems.push(
      problem("NO_SPLIT_VALUES", "split", kind + " split needs values")
    );
    return problems;
  }
  ...
  if (kind === "percentage") {
    const totalPct = sumNumericValues(values);
...
function normalizeSplit(split: SplitRule, participants: string[]): SplitRule {
  ...
  for (let i = 0; i < keys.length; i++) {
    values[keys[i]] = (split.values as Record<string, number | Money>)[keys[i]];
  }
...
function hasDuplicates(list: unknown[]): boolean {
