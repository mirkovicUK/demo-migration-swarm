// expense.ts — the Expense domain model. Depends on money.js (import edge →
...
function normalizeSplit(split: SplitRule, participants: string[]): SplitRule {
  ...
  const srcValues = split.values as Record<string, number | Money>;
  const keys = Object.keys(srcValues);
  for (let i = 0; i < keys.length; i++) {
    values[keys[i]] = srcValues[keys[i]];
  }
