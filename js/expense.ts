...
// Extend Error to carry extra fields
...
// together. Returns an array of ValidationProblem.
export function validateExpense(input: unknown): ValidationProblem[] {
  ...
  // Cast to a record so we can access properties safely
  ...
  const splitProblems = validateSplit(
    inp.split,
    Array.isArray(inp.participants) ? (inp.participants as string[]) : undefined,
    inp.amount
  );
