...
function reviveMoney(m: unknown) {
  if (!m || typeof (m as { amountMinor?: number }).amountMinor !== "number") {
    throw new Error("cannot revive money value");
  }
  return money(Math.round((m as { amountMinor: number }).amountMinor), (m as { currency: string }).currency);
}