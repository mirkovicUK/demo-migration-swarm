...
import {
  ...
  toggleTodo as toggleTodoInStorage,
  removeTodo as removeTodoFromStorage,
  countDone as countDoneInStorage,
  loadTodos,
  saveTodos,
} from "./storage";
...
let todos: ReturnType<typeof loadTodos> = loadTodos();
...
function render() {
  ...
  todos.forEach((todo) => {
    ...
    checkbox.addEventListener("change", () => {
      ...
      todos = toggleTodoInStorage(todos, todo.id);
      saveTodos(todos);
      render();
    });
    ...
    removeBtn.addEventListener("click", () => {
      ...
      todos = removeTodoFromStorage(todos, todo.id);
      saveTodos(todos);
      render();
    });
...
form.addEventListener("submit", (event) => {
  ...
  todos = addTodoToStorage(todos, value);
  saveTodos(todos);
  input.value = "";
  render();
});
