...
import { isSplitKind, type Expense, type Split, type SplitValued } from "./expense.js";
...
export function computeShares(expense: Expense): Record<string, Money> {
  ...
  if (kind === "shares") {
    return byShares(amount, participants, (expense.split as SplitValued).values);
  }
  if (kind === "percentage") {
    return byPercentage(amount, participants, (expense.split as SplitValued).values);
  }
  // exact
  return byExact(amount, participants, (expense.split as SplitValued).values);
}
