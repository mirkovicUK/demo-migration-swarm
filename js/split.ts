...
function byShares(
  amount: Money,
  participants: string[],
  values: Record<string, number | Money> | undefined
): Record<string, Money> {
  const weights = participants.map((pid) => {
    const raw = values ? values[pid] : undefined;
    const w = Number(raw);
...
function byExact(
  ...
  for (let i = 0; i < participants.length; i++) {
    const pid = participants[i];
    const v = values ? values[pid] : undefined;
    shares[pid] = isMoney(v) ? (v as Money) : money(0, amount.currency);
  }
  return shares;
}
