...
export function addTodo(todos: ReadonlyArray<Todo>, text: string): Todo[] {
  return [...todos, createTodo(text)];
}

export function toggleTodo(todos: ReadonlyArray<Todo>, id: number): Todo[] {
  return todos.map((t) => (t.id === id ? { ...t, done: !t.done } : t));
}

export function removeTodo(todos: ReadonlyArray<Todo>, id: number): Todo[] {
  return todos.filter((t) => t.id !== id);
}

export function countDone(todos: ReadonlyArray<Todo>): number {
