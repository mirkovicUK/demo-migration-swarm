...
// framework-free, matching the "vanilla TS, no build step" migration source.
const todoSrc = fs.readFileSync(
  path.join(__dirname, "..", "js", "todo.ts"),
  "utf8"
);
...
test("addTodo appends without mutating the original array", () => {
  const original: Array<{ id: number; text: string; done: boolean }> = [];
